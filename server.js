...
const LEVELS = [
  {
    ...
    lessons:[
      ['Alphabet A–Z',true,'Learn the English alphabet, letter names and simple spelling.',['A','B','C','Hello','Name']],
      ['Colors & Basic Vocabulary',true,'Learn common colors and essential everyday words.',['red','blue','green','book','house']],
      ['Greetings & Introductions',true,'Practice hello, goodbye, please, thank you and simple introductions.',['Hello','Good morning','My name is Hermano','Nice to meet you']],
      ...
      ['Numbers & Simple Questions',false,'Numbers, age, prices and simple questions with what, where and how many.',['one','twenty','How old are you?','How much is it?']],
      ...
      ['Simple Conversations',false,'Build short conversations for home, school, work and everyday situations.',['How are you?','I need help.','Can you repeat, please?']],
...
async function init(){
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS tests(id SERIAL PRIMARY KEY,course_id INT REFERENCES courses(id) ON DELETE CASCADE,slug TEXT UNIQUE NOT NULL,title TEXT NOT NULL,description TEXT,pass_score INT NOT NULL DEFAULT 70,required BOOLEAN DEFAULT TRUE,questions JSONB NOT NULL);
  ...
  for(const level of LEVELS){
    ...
    for(const t of level.tests){const [slug,title,description,pass,questions]=t;await pool.query(`INSERT INTO tests(course_id,slug,title,description,pass_score,required,questions) VALUES($1,$2,$3,$4,$5,TRUE,$6) ON CONFLICT(slug) DO UPDATE SET course_id=EXCLUDED.course_id,title=EXCLUDED.title,description=EXCLUDED.description,pass_score=EXCLUDED.pass_score,questions=EXCLUDED.questions`,[cid,slug,title,description,pass,JSON.stringify(questions)]);}
  }
}
...
app.get('/api/tests/:slug',auth,async(req,res)=>{const t=(await pool.query('SELECT * FROM tests WHERE slug=$1',[req.params.slug])).rows[0];if(!t)return res.status(404).json({error:'Test not found.'});const c=(await pool.query('SELECT * FROM courses WHERE id=$1',[t.course_id])).rows[0];if(!(await levelUnlocked(req.user.id,c.level_order)))return res.status(403).json({error:'This test is locked.'});const p=await courseProgress(req.user.id,c.id);const checkpoint=t.slug==='basic-checkpoint';if(!checkpoint&&p.completed<p.total)return res.status(403).json({error:`Complete all ${p.total} chapters before this test.`});if(checkpoint&&p.completed<3)return res.status(403).json({error:'Complete the first 3 free chapters before this test.'});res.json({id:t.id,slug:t.slug,title:t.title,description:t.description,pass_score:t.pass_score,questions:t.questions.map((x,i)=>({id:i,prompt:x[0],options:x[1]}))});});
app.post('/api/tests/:slug/submit',auth,async(req,res)=>{const t=(await pool.query('SELECT * FROM tests WHERE slug=$1',[req.params.slug])).rows[0];if(!t)return res.status(404).json({error:'Test not found.'});const c=(await pool.query('SELECT * FROM courses WHERE id=$1',[t.course_id])).rows[0];if(!(await levelUnlocked(req.user.id,c.level_order)))return res.status(403).json({error:'This test is locked.'});const p=await courseProgress(req.user.id,c.id);const checkpoint=t.slug==='basic-checkpoint';if(checkpoint&&p.completed<3)return res.status(403).json({error:'Complete the first 3 chapters first.'});if(!checkpoint&&p.completed<p.total)return res.status(403).json({error:`Complete all ${p.total} chapters first.`});const ans=Array.isArray(req.body?.answers)?req.body.answers:[];const correct=t.questions.reduce((n,q,i)=>n+(norm(ans[i])===norm(q[2])?1:0),0);const score=Math.round(correct/t.questions.length*100);const passed=score>=t.pass_score;await pool.query('INSERT INTO test_attempts(user_id,test_id,score,total,passed,answers) VALUES($1,$2,$3,$4,$5,$6)',[req.user.id,t.id,score,t.questions.length,passed,JSON.stringify(ans)]);if(passed&&!checkpoint)await maybeCertificate(req.user.id,c.id);res.json({score,passed,passScore:t.pass_score,correct,total:t.questions.length});});

app.post('/api/upgrade',auth,async(req,res)=>{const plan=String(req.body?.plan||'monthly');const days={weekly:7,monthly:30,annual:365}[plan]||30;await pool.query('UPDATE users SET premium_until=GREATEST(COALESCE(premium_until,NOW()),NOW())+$1::interval WHERE id=$2',[`${days} days`,req.user.id]);res.json({ok:true,mode:'DEMO',message:'Demo upgrade active. Real payment integration can be connected after the payment provider is chosen.'});});
app.post('/api/share',auth,async(req,res)=>{await pool.query('INSERT INTO shares(user_id,lesson_id) VALUES($1,$2)',[req.user.id,req.body?.lessonId||null]);res.json({ok:true});});
